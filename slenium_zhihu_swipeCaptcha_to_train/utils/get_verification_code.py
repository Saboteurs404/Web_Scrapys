...
options = Options()
options.add_experimental_option('excludeSwitchs', ['enable-automatoin'])
...
class get_vericication():
    ...
    def login(self):
        ...
        # 清空并输入用户名
        UserName_Box.clear()
        UserName_Box.send_keys("1782824790@qq.com")

        # 情况并输入密码
        Pwd_Box.clear()
        Pwd_Box.send_keys("19990106.")
    ...
    def saving_img(self):
        ...
        # 截取文件名
        img_list = img.split('/')[-1]
        img_name = ''.join(img_list)

        # 获取图片保存位置:训练集
        path = './images/train/{}'.format(img_name)
        # 测试集：
        # path = './images/test/{}'.format(img_name)
